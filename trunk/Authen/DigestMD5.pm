...
# chy: changed a bit so it works for SIP as well
sub add_digest {
    my $this=shift;
    ...
    my $A1;
    if (defined $pair{'algorithm'} and $pair{'algorithm'} =~ /^MD5-sess$/i) {
        $A1=join (":",
		  md5(join (":", @pair{qw(username realm password)}, )),
		  @pair{qw(nonce cnonce)} );
    } else {
        $A1= join (":", @pair{qw(username realm password)});
    }

    my $meth;
    if (defined $pair{'method'}) {
        $meth = $pair{'method'};
    } else {
	$meth = 'AUTHENTICATE';
    }

    my $A2 = $meth . ':' . $pair{'digest-uri'};
    my $body;
    if (exists $pair{'entity-body'}) {
        $body = $pair{'entity-body'};
    } else {
    ...
    if (defined $pair{'qop'} and $pair{'qop'} =~ /^auth/) {
	$this->{response} =
